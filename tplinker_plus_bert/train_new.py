...
def get_max_length(tokenize, train_data, valid_data):
    # 验证训练集和测试集的token长度
    max_tok_num = 0
    ...
    return max_tok_num
def data_statistics(train_data, valid_data):
    """
    数据统计
    :return:
    :rtype:
...
def check_tagging_decoding(hyper_parameters, train_data, handshaking_tagger, get_tok2char_span_map):
...
def check_data_decode(tokenizer, handshaking_tagger):
    # 查看和验证下dataloader的数据
...
def count_model_parameters(rel_extractor, encoder):
    # 统计参数
...
def test_model_output(rel_extractor, batch_input_ids, batch_attention_mask, batch_token_type_ids, device):
    # test outputs
    rel_extractor.train()
    with torch.no_grad():
...
def train(dataloader, ep, rel_extractor, hyper_parameters, scheduler, logger, config, optimizer):
    """
    开始训练
...
def train_n_valid(train_dataloader, dev_dataloader, optimizer, scheduler, num_epoch, rel_extractor,logger, device, config, metrics, valid_dataloader, model_state_dict_dir):
...
def do_train():
    # 加载config.py中的训练的配置
    config = config_file.train_config
    # 模型训练的超参数
    ...
    if config["logger"] == "wandb":
        ...
        logger = DefaultLogger(config["log_path"], experiment_name, config["run_name"], config["run_id"],
                               hyper_parameters)
    ...
    max_tok_num = get_max_length(tokenize, train_data, valid_data)
    ...
    train_n_valid(train_dataloader, valid_dataloader, optimizer, scheduler, hyper_parameters["epochs"], rel_extractor,logger, device, config, metrics, valid_dataloader,model_state_dict_dir)
